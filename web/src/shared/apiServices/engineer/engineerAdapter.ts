...
export class EngineerAdapter {
  static async signup(data: CompleteRegistrationData) {
    ...
    const response = await axiosInstance.post(ENGINEER_ROUTER_PATHS.SIGNUP, engineerPayload);
  ...
  static async signin(data: LoginFormData) {
     ...
     const response = await axiosInstance.post(ENGINEER_ROUTER_PATHS.SIGNIN, payload);
     return response.data;
  }
