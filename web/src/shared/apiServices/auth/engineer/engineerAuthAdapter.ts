...
export class EngineerAuthAdapter {
    static async signIn(args: { phoneOrEmail: string, password: string }) {
        const response = await axiosInstance.post(ENGINEER_USER_AUTH_ROUTER_PATHS.LOGIN, args);
        return response.data
    }

    static async signup(data: EngineerSignUpData): Promise<unknown> {
        const response = await axiosInstance.post(ENGINEER_USER_AUTH_ROUTER_PATHS.SIGNUP, data);
        return response.data;
    }
