...
const NotificationList: React.FC = () => {
  ...
  const { mutateAsync: markAllAsReadAsync } =
    useAppMarkAllNotificationsAsRead();
  ...
  const renderNotifCard = (notif: NotificationProps, index: number) => (
    <div
      ...
      className={`rounded-md p-3 cursor-pointer border transition-colors relative ${
        !notif.read
    ...
    >
      ...
      <div className="flex flex-col md:flex-row gap-x-4 w-full">
        ...
        <div className="flex flex-col flex-1 w-full pr-8">
          ...
          {!notif.read && <div className="mt-2">{ButtonRender(notif)}</div>}
