import type { SearchEvent, ClickEvent } from "@/dummy_data/engineer_search/searchAnalytics";
...
export const getKeywordFrequency = (searches: SearchEvent[]): Record<string, number> =>
...
export const getSearchTrends = (searches: SearchEvent[]): Record<string, number> =>
  searches.reduce<Record<string, number>>((acc, s) => {
...
export const getCTR = (searches: SearchEvent[], clicks: ClickEvent[]): number =>
  searches.length ? Math.round((clicks.length / searches.length) * 100) : 0;

/** Prepare search history analytics table */
export const getSearchHistory = (searches: SearchEvent[], clicks: ClickEvent[]) => {
