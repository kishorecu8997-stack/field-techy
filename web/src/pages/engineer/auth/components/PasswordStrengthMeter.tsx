...
const PasswordStrengthMeter: React.FC<PasswordStrengthMeterProps> = ({ password }) => {
  ...
  // Define the criteria from your validatePassword function
  const criteria = [
    { name: "8-20 characters", isValid: password.length >= 8 && password.length <= 20 },
  ...
  // Calculate the score based on met criteria
    ...
    <div className="mt-2">
      <div className="flex justify-between items-center mb-1">
        <span className="text-sm font-medium text-gray-700">
          Strength: <strong style={{ color: strengthColor.split('-')[1] }}>{strengthText}</strong>
        </span>
      </div>
      ...
      {/* Optional: Display individual criteria checks 
        Heavy Check Mark && Heavy Ballot Cross Mark
      */}
      <ul className="mt-3 text-xs text-gray-600 space-y-1">
        {criteria.map((criterion) => (
          <li key={criterion.name} className={`flex items-center ${criterion.isValid ? 'text-green-600' : 'text-red-500'}`}>
            <span className="mr-2">
              {criterion.isValid ? '✔' : '✘'} 
            </span>
            {criterion.name}
