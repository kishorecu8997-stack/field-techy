...
export const PasswordOTPVerification = ({
    ...
    const { control, watch, clearErrors, setValue, trigger, setError } = useFormContext();
            ...
            <Popup open={showOTP} onClose={() => setShowOTP(false)}>
                <OTPPage
                    ...
                    onVerifySuccess={() => {
                        ...
                        setVerified(true);
                        // Manually update ref to ensure immediate validation passes
                        verifiedRef.current = true;

                        // Execute parent callback if any
                        onVerifySuccess?.();

                        // ✅ Re-validate the email field with updated verified state
                        const currentValue = watch(name);
                        setValue(name, currentValue, { shouldValidate: true });
                        setValue("otp", otp);

                        // NOTE: The OTP value itself is stored in the 'emailOTP' field 
                        // (or whatever 'name' prop is resolved to in OTPPage) by the OTPInput component.
                        // We do not need to manually set it here as OTPPage handles the input.
